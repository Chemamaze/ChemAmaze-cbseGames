 const uiTranslations = {
        ...
        const additionalGamesRepository = {
    Science: {

        6: [
            {
                 name:"Crossword(Mindful eating)",
            url:"https://drive.google.com/file/d/1YYsArRDm_vTGFtCv_M6xnnw0xzNBGemH/view"
        ...
        7: [
            {
                name: "Maze-Chase (acid-base)",
                url: "https://wayground.com/join/game/U2FsdGVkX18hLhpo7WlXF8rSzBiVEAf2DWxmB2f7JMV9n4mKFPy0Je6Fj%252Faz6p1U?gameType=async"
            },
        ],

        8: [
            {
                name: "Whach-a-mole (Health)",
                url: "https://wayground.com/join/pre-game/running/U2FsdGVkX1%252FDHqLpEPD4osOgKGNdOzWP1PYLIWYJ3N5alGW1ywkoHx3T4Ne5HfCIRk9ShnXayT%252FTR%252BArRebAysDOfQu0Fw3UkD97Dr50AmM%253D/start"
            }
        ]
    },
    ...
    Mathematics: {
        ...
        6: [
            {
                ...
                url: "LINK_FOR_CLASS6_MATH_GAME1"
            }
        ],
        ...
        7: [
            {
                name: "Algebra Puzzle",
                url: "LINK_FOR_CLASS7_MATH_GAME1"
            }
        ],
        ...
        8: [
            {
                name: "Geometry Quest",
                url: "LINK_FOR_CLASS8_MATH_GAME1"
            }
        ]
    }
};
...
function setLanguage(langCode) {
    ...
    if (selectedSubject) {
        ...
        document.getElementById('grade-screen-title').innerText =
            `${subjectLabel} - ${dict.selectClass}`;
    }
...
function loadGames(classId) {
    ...
    const container =
        document.getElementById('games-list-container');

    const title =
        document.getElementById('selected-class-title');

    const dict =
        uiTranslations[currentLang];
    ...
    title.innerText =
        `${subjectLabel} : ${dict.classPrefix} ${classId}`;

    container.innerHTML = '';

    const games =
        gamesRepository[selectedSubject]?.[classId] || [];
    ...
    if (games.length > 0) {
        ...
        games.forEach(game => {
            ...
            gameCard.innerHTML = `
                <div class="chapter-title">
                    ${game.chapter[currentLang]}
                </div>

                <div>
                    ${game.topic[currentLang]}
                </div>
        ...
        const extraGames =
            additionalGamesRepository[selectedSubject]?.[classId];
        ...
        if (extraGames && extraGames.length > 0) {
            ...
            btn.className = "additional-games-btn";

            btn.innerHTML = "🎮 Additional Games";
...
function showAdditionalGames() {
    ...
    const container =
        document.getElementById("additional-games-container");

    container.innerHTML = "";

    const games =
        additionalGamesRepository[selectedSubject]?.[selectedClass] || [];
    ...
    navigateTo("additional-games");
}

function goBack() {

    if (currentScreen === 'additional-games') {
        navigateTo('games');
    }
